.LISAR_AdapLASSO = function(N, TT, Lags, Ydata, lambda1, lambda2, lambda3, gamma.pen, eps1, alpha.pen, reshape) {
  ...
  # adaptive weights list
  # derived from a VAR with the max number of Lags
  adap.weights = .LASSO2(N=N, Lags = Lags, Ydata = Ydata[1:(Lags+1)], lambda3 = 0, eps1 = eps1)[[1]][[1]]
      ...
      for(j2 in 1:length(lambda2)) {
        ...
        for(j3 in 1:length(lambda3)) {
          ...
          while(k2 < Lags) {
            ...
            # times lambda. The diagonal has currently no penalty as it is 
            ...
            while(k1 < N) {
              ...
              adap.weights.current = 1 /(abs(do.call(cbind, adap.weights[sort(b_order_new_store)]))^gamma.pen)
              reg_para_matrix = matrix(1/alpha.pen * (adap.weights.current) * lambda3[j3], N, N * k2)
              if (k2 > 1) {
                which_elements = unlist(Map(function(x, y) relist(unlist(x) + y, skeleton = x), (1:length(iset_new_store_list) - 1) * N, iset_new_store_list))
                reg_para_matrix[,sort(which_elements)] = alpha.pen * (adap.weights.current)[,sort(which_elements)] *  lambda3[j3]
              }
              reg_para_matrix[,sort(iset_current) + (N * (k2-1))] = alpha.pen * (adap.weights.current)[,sort(iset_current) + (N * (k2-1))] *  lambda3[j3]
              for (jjj in 1:k2) {
                diag(reg_para_matrix[,(1:N) + (N*(jjj-1))]) = alpha.pen * diag((adap.weights.current)[,(1:N) + (N*(jjj-1))]) * lambda3[j3]
              }
  ...
  close(pb)
  lambdas = list(lambda1, lambda2, lambda3, alpha.pen, gamma.pen)
